...
def gen_randomly_samples_for_faiss_train_random10000():
    prefix = 'HERE_'
    ...
    d2 = 256
    version = 'v2'
    save_dir = f'/data/Jiang_Lab/Data/Zisha_Zhong/temp_20240208_hidare/search_results_{version}/'
    ...
    save_filename = os.path.join(save_dir, f'randomly_background_samples_for_train_{project_name}_{prefix}{backbone}V2.pkl')

    feats_filenames = glob.glob(os.path.join(feats_dir, '*.pt'))
    selected_inds = np.random.choice(np.arange(len(feats_filenames)), size=min(200, len(feats_filenames)), replace=False)
    ...
    for _, i in enumerate(selected_inds):
        ...
        embeddings1.append(feats.reshape(-1, d2))

        feats = feats0.numpy()
        feats /= np.linalg.norm(feats, axis=1)[:, None]
        embeddings2.append(feats.reshape(-1, d1))
        count += 1
    ...
    with open(save_filename, 'wb') as fp:
        pickle.dump({'HERE_ProvGigaPath': randomly_selected_data1, 'ProvGigaPath': randomly_selected_data2}, fp)



def gen_randomly_samples_for_faiss_train():
    prefix = 'HERE_'
    project_name = 'KenData'
    project_name = 'ST'
    backbone = 'ProvGigaPath'
    ...
    save_filename = os.path.join(save_dir, f'randomly_background_samples_for_train_{project_name}_{prefix}{backbone}.pkl')

    feats_filenames = glob.glob(os.path.join(feats_dir, '*.pt'))
    selected_inds = np.random.choice(np.arange(len(feats_filenames)), size=100, replace=False)
    ...
    for i in selected_inds:
        ...
        feats0 = torch.load(feats_filenames[i])
        selected_inds2 = np.random.choice(np.arange(len(feats0)), size=feats0.shape[0]//2 if feats0.shape[0]<100 else 100, replace=False)
        feats = feats0[selected_inds2].reshape(len(selected_inds2), -1)
        feats = feats.float() @ attention_net_W + attention_net_b
        feats = feats.numpy()
        feats /= np.linalg.norm(feats, axis=1)[:, None]
        embeddings.append(feats.reshape(-1, d))

    embeddings = np.concatenate(embeddings, axis=0)
    print(project_name, embeddings.shape)
    randomly_selected_data[project_name] = {'embeddings': embeddings}
    
    with open(save_filename, 'wb') as fp:
        pickle.dump(randomly_selected_data, fp)



def add_feats_to_faiss(): # HERE_ProvGigaPath
    prefix = 'HERE_'
    project_names = ['KenData']
    project_names = ['ST']
    ...
    train_data_filename = os.path.join(save_dir, f'randomly_background_samples_for_train_{project_name}_{prefix}{backbone}.pkl')
    train_data_filename = os.path.join(save_dir, f'randomly_background_samples_for_train_{project_name}_{prefix}{backbone}V2.pkl')
    ...
    if not os.path.exists(train_data_filename):
        ...
        # gen_randomly_samples_for_faiss_train
    ...
    with open(train_data_filename, 'rb') as fp:
        ...
        # train_data_float32 = np.concatenate([v['embeddings'] for v in list(randomly_train_data.values())])
    ...
    for params in faiss_types_sub:
        ...
        for proj_id, project_name in enumerate(project_names):
            ...
            for file_index, h5filename in enumerate(h5filenames):
                ...
                if project_name == 'ST':
                    allfeats_dir = '/data/zhongz2/temp29/debug/debug_results/ngpus2_accum4_backboneProvGigaPath_dropout0.25/analysis/ST/ProvGigaPath/feat_before_attention_feat/cached_data'
                    single_cache_filename = os.path.join(allfeats_dir, 'slide_{}_cache.pkl'.format(svs_prefix))
                    ...
                    feats = feats.float() @ attention_net_W + attention_net_b
                    feats = feats.numpy().reshape(-1, d)

                feats /= np.linalg.norm(feats, axis=1)[:, None]
                # allfeats.append(feats.numpy())
                if file_index % 500 == 0:
...
def add_feats_to_faiss():
    ...
    save_dir = f'/data/Jiang_Lab/Data/Zisha_Zhong/temp_20240208_hidare/search_results_{version}/'
    os.makedirs(save_dir, exist_ok=True)
    feats_dir = f'/data/zhongz2/KenData_256/feats/{backbone}/pt_files'
    patches_dir = '/data/zhongz2/KenData_256/patches'

    train_data_filename = os.path.join(save_dir, f'randomly_background_samples_for_train_{project_name}_{backbone}.pkl')
    ...
    for params in faiss_types:
        ...
        for proj_id, project_name in enumerate(project_names):
            save_filename = f'{faiss_bins_dir}/all_data_feat_before_attention_feat_faiss_{faiss_type}_{project_name}_{backbone}.bin'
            if project_name == 'KenData' and faiss_type == 'IndexFlatL2':
                continue
            if project_name == 'TCGA-COMBINED' and faiss_type == 'IndexFlatL2':
                continue
    ...
    for params in faiss_types_sub:
        ...
        for proj_id, project_name in enumerate(project_names):
            ...
            for file_index, h5filename in enumerate(h5filenames):
                ...
                feats = torch.load(os.path.join(feats_dir, svs_prefix+'.pt')).numpy().reshape(-1, d)
