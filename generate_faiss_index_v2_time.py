...
def add_feats_to_faiss(project_name='KenData', backbone='ProvGigaPath', HERE_ckpt_filename=None, save_dir=None, train_data_filename=None, version='', bigmem=0): # HERE_ProvGigaPath
    ...
    print('faiss_types_sub', faiss_types_sub)
...
def get_all_scales_20241219_CPTAC():
    ...
    os.makedirs(save_dir, exist_ok=True)
    all_results = {}
    with open(f'{save_dir}/all_scales_20240813_newKenData.pkl', 'rb') as fp:
...
def update_note_and_external_links_forCPTAC():
    ...
    for proj_id, project_name in enumerate(project_names):
        ...
        for svs_prefix_id, h5filename in enumerate(h5filenames):
            ...
            if project_name == 'CPTAC':
                ...
                if len(dff) > 0:
                    note += '\nhttps://www.cancerimagingarchive.net/collection/cptac-{}/\n'.format(dff['cancer_type'].values[0].lower())
...
if __name__ == '__main__':
    ...
    save_dir=f'/data/zhongz2/test_indexing_time/faiss_related{version}'
    ...
    with open(os.path.join(save_dir, 'time.txt'), 'w') as fp:
        fp.write("time: {}".format(elapsed_time))
