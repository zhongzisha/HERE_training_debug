...
FEATS_DIR=/lscratch/$SLURM_JOB_ID/HERE_${MODEL_NAME}
...
for MODEL_NAME in "CONCH" "UNI" "ProvGigaPath"; do  # "UNI" "CONCH" "ProvGigaPath"
for SPLIT in 0 1 2 3 4; do

CSV_PATH="./splits/trainval-${SPLIT}.csv"
SAVE_PREFIX="TCGA_trainval${SPLIT}"
sbatch --job-name TRAIN --nodes=1 --ntasks-per-node=1 --cpus-per-task=4 --partition=gpu \
--gres=gpu:v100x:1,lscratch:100 --mem=100gb --time=24:00:00 \
job_test_model_TCGAtrainval.sh ${MODEL_NAME} ${CSV_PATH} ${SAVE_PREFIX}
