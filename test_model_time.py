...
def main(args):
    ...
    for index, row in sub_df.iterrows():
        ...
        if not os.path.exists(backbone_feature_filename):
            ...
            feature_extraction_time = time.time() - time_start1
        ...
        attention_time = time.time() - time_start2
        ...
        all_times.append((svs_prefix, len(all_coords), feature_extraction_time, attention_time, time_elapsed-3))

    
    time_df = pd.DataFrame(all_times, columns=['svs_prefix', 'num_patches', 'feature_extraction_time', 'attention_time', 'processing_time'])
    time_df.to_csv(os.path.join(args.feat_dir, 'time.csv'))
