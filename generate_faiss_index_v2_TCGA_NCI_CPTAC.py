...
if __name__ == '__main__':
    ...
    save_dir=f'/data/zhongz2/temp_20241205_TCGA_NCI_CPTAC/faiss_related{version}'
    os.makedirs(save_dir, exist_ok=True)
...
def make_symbol_links():
    
    import os,shutil,glob
    from tqdm import tqdm
    os.makedirs('/data/zhongz2/TCGA_NCI_CPTAC_256/svs', exist_ok=True)
    os.makedirs('/data/zhongz2/TCGA_NCI_CPTAC_256/patches', exist_ok=True)
    os.makedirs('/data/zhongz2/TCGA_NCI_CPTAC_256/feats/CONCH/pt_files', exist_ok=True)
    os.makedirs('/data/zhongz2/TCGA_NCI_CPTAC_256/feats/UNI', exist_ok=True)
    os.makedirs('/data/zhongz2/TCGA_NCI_CPTAC_256/feats/ProvGigaPath', exist_ok=True)

    proj_names = ['TCGA-COMBINED', 'KenData_20240814', 'CPTAC']
    for proj in proj_names:
        patch_files = glob.glob(os.path.join('/data/zhongz2/{}_256/patches/*.h5'.format(proj)))
        for f in tqdm(patch_files):
            svs_prefix = os.path.splitext(os.path.basename(f))[0]
            # svs
            os.system('ln -sf "{}" "{}"'.format(
                os.path.join(f'/data/zhongz2/{proj}_256/svs/{svs_prefix}.svs'),
                os.path.join(f'/data/zhongz2/TCGA_NCI_CPTAC_256/svs/')
            ))
            # patch
            os.system('ln -sf "{}" "{}"'.format(
                os.path.join(f'/data/zhongz2/{proj}_256/patches/{svs_prefix}.h5'),
                os.path.join(f'/data/zhongz2/TCGA_NCI_CPTAC_256/patches/')
            )) 
            # feat
            for backbone in ['CONCH']:
                os.system('ln -sf "{}" "{}"'.format(
                    os.path.join(f'/data/zhongz2/{proj}_256/feats/{backbone}/pt_files/{svs_prefix}.pt'),
                    os.path.join(f'/data/zhongz2/TCGA_NCI_CPTAC_256/feats/{backbone}/pt_files/')
                ))           
