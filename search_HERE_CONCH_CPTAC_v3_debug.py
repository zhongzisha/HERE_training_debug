...
def load_model_config_from_hf(model_id: str):
    cached_file = f'{DATA_DIR}/faiss_relatedV20240908_intersection/ProvGigaPath/config.json'
...
def create_model():
    ...
    load_checkpoint(model, f'{DATA_DIR}/faiss_relatedV20240908_intersection/ProvGigaPath/pytorch_model.bin')
...
for search_backbone in backbones:
    ...
    if search_backbone == 'HERE_PLIP':
        models_dict[search_backbone]['feature_extractor'] = CLIPModel.from_pretrained(f"{DATA_DIR}/faiss_relatedV20240908_intersection/vinid_plip")
        models_dict[search_backbone]['image_processor_or_transform'] = CLIPProcessor.from_pretrained(f"{DATA_DIR}/faiss_relatedV20240908_intersection/vinid_plip")
        models_dict[search_backbone]['attention_model'] = AttentionModel()
        models_dict[search_backbone]['state_dict'] = torch.load(f"{DATA_DIR}/faiss_relatedV20240908_intersection/snapshot_66_HERE_PLIP.pt", map_location=DEVICE) #, weights_only=True)
    elif search_backbone == 'HERE_CONCH':
        from conch.open_clip_custom import create_model_from_pretrained
        models_dict[search_backbone]['feature_extractor'], models_dict[search_backbone]['image_processor_or_transform'] = create_model_from_pretrained('conch_ViT-B-16', checkpoint_path=f'{DATA_DIR}/faiss_relatedV20240908_intersection/CONCH_weights_pytorch_model.bin')
        models_dict[search_backbone]['attention_model'] = AttentionModel(backbone='CONCH')
        models_dict[search_backbone]['state_dict'] = torch.load(f"{DATA_DIR}/faiss_relatedV20240908_intersection/snapshot_53_HERE_CONCH.pt", map_location=DEVICE) #, weights_only=True)
        ...
        models_dict[search_backbone]['state_dict'] = torch.load(f"{DATA_DIR}/faiss_relatedV20240908_intersection/snapshot_39_HERE_ProvGigaPath.pt", map_location=DEVICE) #, weights_only=True)
    elif search_backbone == 'HERE_UNI':
        models_dict[search_backbone]['feature_extractor'] = timm.create_model(
            "vit_large_patch16_224", img_size=224, patch_size=16, init_values=1e-5, num_classes=0, dynamic_img_size=True
        )
        # models_dict[search_backbone]['feature_extractor'].load_state_dict(torch.load(f"{DATA_DIR}/faiss_relatedV20240908_intersection/UNI_pytorch_model.bin", map_location="cpu", weights_only=True), strict=True)
        models_dict[search_backbone]['feature_extractor'].load_state_dict(torch.load(f"{DATA_DIR}/faiss_relatedV20240908_intersection/UNI_pytorch_model.bin", map_location=DEVICE), strict=True)
        ...
        models_dict[search_backbone]['state_dict'] = torch.load(f"{DATA_DIR}/faiss_relatedV20240908_intersection/snapshot_58_HERE_UNI.pt", map_location=DEVICE)#, weights_only=True)
...
for backbone in backbones:
    ...
    for faiss_type in faiss_types:
        ...
        for project_name in project_names:
            faiss_bin_filename = f"{DATA_DIR}/faiss_relatedV20240908_intersection/faiss_bins/all_data_feat_before_attention_feat_{faiss_type}_{project_name}_{backbone}.bin"
...
# with open(f'{DATA_DIR}/faiss_relatedV20240908_intersection/randomly_1000_data_with_PLIP_ProvGigaPath_CONCH_20240814.pkl', 'rb') as fp: # with HERE_ProvGigaPath, with CONCH, random normal distribution
...
for method in ['HERE_CONCH']: #['HERE_ProvGigaPath', 'HERE_CONCH', 'HERE_PLIP', 'HERE_UNI']:
    ...
    for project_name in project_names: #['KenData_20240814', 'ST_20240903', 'TCGA-COMBINED']:
        if project_name == 'ST_20240903':
            ...
            version = 'V6'
        filename = f'{DATA_DIR}/faiss_relatedV20240908_intersection/randomly_background_samples_for_train_{project_name}_{method}{version}.pkl'
...
def main():
    ...
    files = glob.glob('/data/zhongz2/CPTAC/yottixel_bobs/patches_intersection/*/patch1024/top0.png')
    ...
    existed_prefixes = [os.path.splitext(os.path.basename(f))[0] for f in glob.glob(os.path.join(save_root, 'retrieved_results', '*.pkl'))]
    files = [f for f in files if f.split('/')[-3] not in existed_prefixes]
    ...
    for i, f in enumerate(files):
        ...
        if 0 < len(final_response) < topn:
            print('not good results for {}'.format(f))
        ...
        for ri, (svs_prefix, item) in enumerate(final_response.items()):
            ...
            query_results.append((query_prefix, svs_prefix, ri, dists_in_slide.min(), dists_in_slide, coords))
        ...
        # all_results[query_prefix] = (query_prefix, gt, majority_vote_pred, top_n_labels, top_n_dists)
        # all_results_per_slide[query_prefix] = query_results

        with open(os.path.join(save_root, 'retrieved_results', query_prefix+'.pkl'), 'wb') as fp:
            pickle.dump({
                'all_results': (query_prefix, gt, majority_vote_pred, top_n_labels, top_n_dists),
                'all_results_per_slide': query_results
            }, fp)
    ...
    # with open(os.path.join(save_root, 'all_results_{}_{}.pkl'.format(os.environ['SLURM_JOB_ID'], idr_torch.rank)), 'wb') as fp:
    #     pickle.dump({
    ...
    # with open(os.path.join(save_root, 'classification_report_{}.txt'.format(idr_torch.rank)), 'w') as fp:
