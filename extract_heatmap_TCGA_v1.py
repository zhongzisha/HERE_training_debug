...
def main():
    ...
    for f in needtodo_files:
        ...
        #     print(f'error {f}')
            ...
            # generate thumbnails
            time.sleep(1)
            thum1 = slide.get_thumbnail(size=(1000, 1000))
            slide2 = openslide.open_slide(save_filename)
            thum2 = slide2.get_thumbnail(size=(1000, 1000))
            thum = np.concatenate([np.array(thum1), np.array(thum2)], axis=1)
            cv2.imwrite(save_filename.replace('.tif', '_thum.jpg'), thum[:,:,::-1])
