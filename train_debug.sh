...
NUM_GPUS=2
GRES_STR="--gres=gpu:v100x:${NUM_GPUS},lscratch:400"
FINAL_SAVE_ROOT=/data/zhongz2/temp29/debug/results_20241215_e100_debug
...
for SPLIT_NUM in 0 1 2 3 4; do     #  0 1 2 3 4
    for ACCUM_ITER in 4; do # 4 8
        for i in ${!BACKBONES[@]}; do
            ...
            sbatch ${GRES_STR} train_debug.sh ${NUM_GPUS} ${SPLIT_NUM} ${ACCUM_ITER} ${FINAL_SAVE_ROOT} ${BACKBONE} ${DROPOUT} ${TCGA_ROOT_DIR}
            sleep 2
        done
    done
done
